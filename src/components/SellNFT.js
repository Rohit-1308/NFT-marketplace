...
export default function SellNFT() {
  ...
  const onChangeFile = async (e) => {
    ...
    try {
      ...
      if (response.success == true) {
        console.log(
          `Uploaded Image to pinata successfully: ` + response.pinataUrl
        );
  ...
  const uploadDataToIPFS = async () => {
    ...
    const nftJson = { name, description, price };

    try {
      const response = await uploadJSONToIPFS(nftJson);
      if (response.success === true) {
        console.log(`Uploaded successfully to Pinata ${response}`);
  ...
  const listNft = async (e) => {
    e.preventDefault();
    try {
      const metadata = await uploadDataToIPFS();
